...
test("a criação da proposta usa o snapshot comercial canônico, não o preço técnico do produto", () => {
  const route = readFileSync(new URL("../app/api/[...path]/route.ts", import.meta.url), "utf8");
  ...
  assert.match(postSection, /buildCommercialSnapshot\(\{/);
  assert.match(postSection, /sku: body\.commercial_sku/);
  assert.match(postSection, /commercial_override_confirmed/);
  assert.doesNotMatch(postSection, /const base = Number\(product\.base_price\)/);
});
