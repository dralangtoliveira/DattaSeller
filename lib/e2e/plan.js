...
import { DECLARED_REF_ENV } from "./target-guard.js";
...
export const REQUIRED_ENV = ["DS_E2E_BASE_URL", "DS_E2E_EMAIL", "DS_E2E_PASSWORD", "NEXT_PUBLIC_SUPABASE_URL", DECLARED_REF_ENV, "NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY", "DS_E2E_EMAIL_TO", "DS_E2E_CONFIRM"];
...
/**
 * Run id canônico do E2E. Aceita o valor injetado pelo operador/CI
 * (`DS_E2E_RUN_ID`) e devolve `null` quando não há valor utilizável — nesse caso
 * o runner gera o próprio id (timestamp). Mantém `slug = e2e-<runId>`.
 */
export function e2eRunId(value) {
  const clean = String(value ?? "").toLowerCase().replace(/[^a-z0-9]/g, "");
  return clean ? clean.slice(0, 40) : null;
}

/**
 * Candidato canonico do E2E. `POST /api/prospects` exige `source_url` público
 * (a mesma guarda de `saveProspect` que vale para o fluxo real), portanto a
 * fixture precisa declará-lo — sem isso o passo `prospect` responde
 * `storage_unavailable` e a cadeia inteira para antes de existir o lead.
 */
export function e2eProspectCandidate({ slug, runId, emailTo }) {
  const sourceUrl = `https://e2e-${runId}.example/`;
  return {
    slug,
    nome: `E2E ${runId}`,
    ...
    email: emailTo,
    site_antigo: sourceUrl,
    instagram_url: `instagram.com/e2e${runId}`,
    source_url: sourceUrl,
  };
}

/**
 * Cabeçalhos das chamadas do E2E. Um Preview protegido por Vercel Authentication
 * responde 401 (`{"protection":{"vercel_auth_enabled":true}}`) antes de chegar ao
 * app; o segredo oficial de *Protection Bypass for Automation* entra por
 * `DS_E2E_BYPASS` e nunca é registrado em log nem em documento.
 */
export function e2eHeaders({ base, cookie = "", bypass = "" } = {}) {
  const headers = { "Content-Type": "application/json", Origin: base };
  if (cookie) headers.Cookie = cookie;
  if (bypass) headers["x-vercel-protection-bypass"] = String(bypass);
  return headers;
}
