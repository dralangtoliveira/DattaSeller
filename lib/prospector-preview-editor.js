// Recovered from poc/dattaseller-local/skills/prospector-redesign/references/editor-visual.md.
// This layer is deliberately injected only in the editor representation. Stored previews stay clean.
const editorLayer = `<!-- PROSPECTOR-EDITOR-START -->
<style id="pe-style">
...
<div id="pe-bar"><strong>Modo edição</strong><span>Clique em textos para editar · clique em imagens para trocar</span><button id="pe-export" type="button">Exportar página</button></div>
...
(function(){
  ...
  document.querySelectorAll(TEXT).forEach(function(el){
    ...
    if(el.children.length===0||el.childElementCount<=1){
      el.addEventListener('click',function(e){if(el.tagName==='A'||el.tagName==='BUTTON')e.preventDefault();el.setAttribute('contenteditable','true');el.focus()});
  ...
  fileInput.addEventListener('change',function(){var f=fileInput.files[0];if(!f||!currentImg)return;var r=new FileReader();r.onload=function(){currentImg.src=r.result;if(currentImg.srcset)currentImg.removeAttribute('srcset')};r.readAsDataURL(f);fileInput.value=''});
  document.getElementById('pe-export').addEventListener('click',function(){
    ...
    var blob=new Blob([html],{type:'text/html'});var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='index.html';a.click();
  });
})();
</script>
<!-- PROSPECTOR-EDITOR-END -->`;

export function withProspectorEditor(content) {
  const page = String(content ?? "");
  if (page.includes("PROSPECTOR-EDITOR-START")) return page;
  return /<\/body\s*>/i.test(page) ? page.replace(/<\/body\s*>/i, `${editorLayer}</body>`) : `${page}${editorLayer}`;
}
