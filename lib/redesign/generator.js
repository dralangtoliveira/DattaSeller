...
function ctaFor(contacts = {}) {
  const zap = contacts.whatsapp?.[0]?.value ? whatsappLink(contacts.whatsapp[0].value) : "";
  ...
  if (telefone) return { href: `tel:+${telefone}`, label: "Ligar agora", kind: "phone", source: contacts.phone[0] };
...
export function generateRedesign({ lead = {}, site = {}, texts = {}, assets = [], palette = [], contacts = {}, diagnosis = null, collectedAt = null } = {}) {
  ...
  const contatoLinhas = [
    ...
    telefone ? `<p class="contato-linha"><b>Telefone</b><a href="tel:+${escapeHtml(telefone)}">${escapeHtml(telefone)}</a></p>` : "",
