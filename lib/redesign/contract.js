...
/**
 * `context_url` é a única referência de rede que o job pode carregar. Ela só é
 * aceita como **caminho relativo** do CRM configurado: URL absoluta, host
 * alternativo, esquema (`http:`, `file:`, `data:`, `javascript:`) ou caminho
 * protocol-relative (`//host`) são recusados aqui, antes de qualquer fetch.
 */
export function parseContextPath(value) {
  const raw = clean(value);
  if (!raw) return null;
  if (raw.includes("\\")) throw new RedesignError("redesign_invalid_context_url", "context_url não pode conter barra invertida.");
  if (!raw.startsWith("/")) throw new RedesignError("redesign_invalid_context_url", "context_url precisa ser um caminho relativo do CRM (ex.: /api/worker/context?lead_slug=...).");
  if (raw.startsWith("//")) throw new RedesignError("redesign_invalid_context_url", "context_url não pode apontar para outro host (//host).");
  if (/^\/[a-z][a-z0-9+.-]*:/i.test(raw)) throw new RedesignError("redesign_invalid_context_url", "context_url não pode conter esquema de URL.");
  return raw;
}

/** Constrói o destino do contexto fixando a origem configurada do CRM. */
export function resolveContextUrl(contextPath, apiUrl) {
  const caminho = parseContextPath(contextPath);
  if (!caminho) return null;
  if (!apiUrl) throw new RedesignError("redesign_context_unavailable", "DATTASELLER_API_URL não está configurada para resolver o contexto do CRM.");
  let base;
  try {
    base = new URL(String(apiUrl));
  } catch {
    throw new RedesignError("redesign_context_unavailable", "DATTASELLER_API_URL inválida.");
  }
  const destino = new URL(caminho, base);
  if (destino.origin !== base.origin) {
    throw new RedesignError("redesign_context_origin_mismatch", "context_url tentou sair da origem configurada do CRM.", { origin: destino.origin, esperado: base.origin });
  }
...
export function parseRedesignJob(body = {}) {
  ...
  return {
    ...
    context_url: parseContextPath(body.context_url ?? body.contextUrl),
