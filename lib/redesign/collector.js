...
export function extractSiteAssets({ html, finalUrl, status = 200, contentType = "text/html", bytes = 0, checkedAt, known = {} }) {
  ...
  const phones = [...new Set([...body.matchAll(/tel:([+0-9()\s.\-]{8,25})/gi)].map((match) => digits(match[1])).filter((value) => value.length >= 10))];
  ...
  const contacts = {
    phone: [
      ...phones.map((value) => ({ value, source: "site publico do lead", source_url: base, evidence: "link tel:", collected_at: checkedAt })),
      ...(known.telefone && !phones.includes(digits(known.telefone)) ? [{ value: digits(known.telefone), source: known.source ?? "crm", source_url: known.source_url ?? null, evidence: "contato confirmado no CRM", collected_at: checkedAt }] : []),
    ],
    whatsapp: whatsapp
      ? [{ value: whatsapp, source: "site publico do lead", source_url: base, evidence: "link wa.me", collected_at: checkedAt }]
