...
export function createRedesignWorker({ fetchImpl = fetch, resolveHost, now = () => new Date(), contextProvider = null, timeoutMs, requireContext = false } = {}) {
  const jobs = new Map();
  ...
  async function buildArtifact(job) {
    ...
    try {
      ...
      contextoIndisponivel = error;
      warnings.push({ code: error?.code ?? "redesign_context_unavailable", message: `Contexto do CRM indisponível: ${error?.message ?? error}` });
    }
    // Em modo remoto/VPS o contexto do CRM é obrigatório: sem ele o redesign não
    // pode ser tratado como execução comercial válida.
    if (requireContext && !contexto) {
      throw new RedesignError("redesign_context_required", "Contexto do CRM é obrigatório no modo worker remoto e não pôde ser obtido.", { causa: contextoIndisponivel?.code ?? "redesign_without_crm_context" });
    }
    ...
    const gerado = generateRedesign({
      lead: { slug: job.lead_slug, nome: lead.nome ?? coleta.site.title, cidade: lead.cidade ?? null, end_cliente: lead.end_cliente ?? null },
      site: coleta.site,
      texts: coleta.texts,
    ...
    const artifact = {
      ...
      generation_metadata: {
        ...
        required_context: requireContext,
