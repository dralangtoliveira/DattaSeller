...
export async function diagnoseSite(url, { fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT_MS, contact = DEFAULT_CONTACT, now = () => new Date() } = {}) {
  ...
  const redirects = [];
  try {
    response = await fetchImpl(String(url), { signal: controller.signal, redirect: "follow", headers: { "User-Agent": contact, Accept: "text/html,application/xhtml+xml", "Accept-Language": "pt-BR,pt;q=0.9,en;q=0.8" } });
  } catch (error) {
  ...
  if (finalUrl !== String(url)) redirects.push({ de: String(url), para: finalUrl });
  return { url: String(url), ...analyzeSite({ finalUrl, status, contentType, html, bytes: html.length, redirects, checkedAt: now().toISOString() }) };
}
