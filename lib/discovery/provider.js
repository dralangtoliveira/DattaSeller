...
/**
 * Regra explícita da busca (não confundir com "devolve a quantidade pedida"):
 * a quantidade alvo é a referência de trabalho do operador; a busca devolve até
 * o limite de candidatos, priorizando quem tem contato público.
 */
export const DISCOVERY_QUANTITY_RULE = "O retorno é limitado pelo limite de candidatos (padrão 25, máximo 50), que é um teto: quantidade alvo 5 com limite 25 devolve até 25 empresas, priorizando as que têm contato público. A quantidade alvo (padrão 10) é a referência de trabalho do operador e dimensiona a amostra consultada, não o teto.";
 ...
 */
async function callProvider({ url, init, fetchImpl, timeoutMs, label, resolveHost }) {
  const { signal, done } = timeoutSignal(timeoutMs);
  let outcome;
  try {
    // Endereço do provedor também passa pela proteção SSRF (host literal,
    // resolução DNS e cada salto de redirect).
    outcome = await guardedFetch(url, { fetchImpl, resolveHost, signal, label, ...init });
  } catch (error) {
    ...
    if (String(error?.code ?? "").startsWith("ssrf_")) throw new DiscoveryError(error.code, error.message, error.details);
 ...
 */
export async function geocodePlace(cidade, { fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT_MS, contact = DEFAULT_CONTACT, url = process.env.DATTASELLER_DISCOVERY_NOMINATIM_URL || DEFAULT_NOMINATIM_URL, resolveHost } = {}) {
  ...
  const payload = await callProvider({ url: endpoint.toString(), init: { headers: { "User-Agent": contact, "Accept-Language": "pt-BR,pt;q=0.9,en;q=0.8" } }, fetchImpl, timeoutMs, label: "Nominatim", resolveHost });
...
export async function discoverCompanies({
  ...
  // O limite de candidatos é o teto do retorno; a quantidade alvo dimensiona a
  // amostra consultada (nunca abaixo do teto nem menor que 20 para ter escolha).
  const teto = normalizeLimit(limite, DISCOVERY_DEFAULT_LIMIT);
  const { strategy, categoria, selector, warning } = nicheSelector(nicho);
  const resolved = place ?? (await geocodePlace(cidade, { fetchImpl, timeoutMs, contact, resolveHost }));
  ...
  const amostra = Math.min(DISCOVERY_MAX_LIMIT, Math.max(teto, alvo, 20));
  ...
  return {
    ...
    warning,
    query: { nicho: clean(nicho), cidade: clean(cidade), quantidade_alvo: alvo, limite_candidatos: teto, regra: DISCOVERY_QUANTITY_RULE },
    place: resolved,
