...
export function buildFollowUpDraft(parent = {}, { sellerName = "", days, referenceDate = new Date() } = {}) {
  ...
  const publicUrl = String(parent?.body ?? "").match(/https:\/\/[^\s]+\/p\/[A-Za-z0-9_-]{43}/)?.[0];
  if (!publicUrl) throw new Error("public_proposal_url_required");
  return {
    subject: /^re:/i.test(subject) ? subject : `Re: ${subject}`,
    body: [`Olá,`, `Retomo a proposta enviada${sentAt ? ` em ${sentAt}` : ""}: ${publicUrl}`, "Se este não for o momento, basta me avisar.", `Atenciosamente, ${signature || "Equipe Datta360°"}`].join("\n"),
