...
const FOLLOW_UP_ELIGIBLE_STATUSES = ["sent", "delivered_simulated", "no_reply"];
...
export function isFollowUpDue(parent = {}, referenceDate = new Date(), days = FOLLOW_UP_DEFAULT_DAYS) {
  if (!canScheduleFollowUp(parent.status)) return false;
  return Date.parse(followUpDueAt(days, parent.updated_at || parent.created_at)) <= new Date(referenceDate).getTime();
}
...
export function buildFollowUpDraft(parent = {}, { sellerName = "", days, referenceDate = new Date() } = {}) {
  ...
  const proposalUrl = String(parent?.body ?? "").match(/https:\/\/[^\s]+\/p\/[A-Za-z0-9_-]{43}/)?.[0];
  if (!proposalUrl) throw new Error("public_proposal_url_required");
  return {
    subject: /^re:/i.test(subject) ? subject : `Re: ${subject}`,
    body: [
      "Olá,",
      `Retomo o contato sobre "${subject}"${sentAt ? ` enviado em ${sentAt}` : ""}.`,
      proposalUrl ? `A proposta continua disponível aqui: ${proposalUrl}` : "Se fizer sentido, respondo com os próximos passos ou ajusto a proposta.",
      `Se fizer sentido, responda a este e-mail. ${signature || "DattaSeller"}`,
    ].join("\n"),
