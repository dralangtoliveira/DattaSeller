...
export function diagnosisFactsFromCriteria(value) {
  ...
  for (const entry of top) {
    ...
    for (const evidence of evidences) {
      ...
      const observed = evidence.observacao || evidence.detail || evidence.value;
      if (observed) push(observed);
      else if (evidence.criterio && evidence.valor) push(`${evidence.criterio}: ${evidence.valor}`);
    }
  }
