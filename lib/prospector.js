...
// Campo ausente (null/undefined/vazio) não é domínio: sem esta guarda, `new URL("https://undefined")`
// produzia o domínio literal "undefined" e dois leads sem site eram deduplicados como se fossem o mesmo.
export const normalizeUrl = (v, domain = false) => { const raw = String(v ?? "").trim(); if (!raw) return ""; try { const u = new URL(/^https?:/i.test(raw) ? raw : `https://${raw}`); const h = u.hostname.toLowerCase().replace(/^www\./, ""); return domain ? h : `${h}${u.pathname.replace(/\/$/, "")}`; } catch { return ""; } };
