...
export async function loadPlaywright({ modulePath = process.env.DATTASELLER_PLAYWRIGHT_MODULE ?? "" } = {}) {
  try {
    ...
    // `@playwright/test` já fornece o mesmo browser runtime e é a única
    // dependência declarada pelo projeto. Instalações especiais usam o caminho
    // explícito acima, sem introduzir uma dependência opcional no bundle.
    let modulo;
    if (entrada) modulo = await import(pathToFileURL(entrada).href);
    else modulo = await import("@playwright/test");
