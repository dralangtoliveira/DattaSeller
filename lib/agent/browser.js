...
export async function loadPlaywright({ modulePath = process.env.DATTASELLER_PLAYWRIGHT_MODULE ?? "" } = {}) {
  try {
    ...
    // `@playwright/test` já fornece o mesmo browser runtime e é a dependência
    // declarada pelo projeto. Só tenta o pacote separado em instalações legadas.
    let modulo;
    if (entrada) modulo = await import(pathToFileURL(entrada).href);
    else {
      try { modulo = await import("playwright"); }
      catch { modulo = await import("@playwright/test"); }
    }
