...
async function fetchText(url, { fetchImpl, timeoutMs, accept = "text/html,application/xhtml+xml", contact }) {
  const { signal, done } = timeoutSignal(timeoutMs);
  try {
    const response = await fetchImpl(url, { signal, redirect: "follow", headers: { "User-Agent": contact, Accept: accept, "Accept-Language": "pt-BR,pt;q=0.9,en;q=0.8" } });
...
export async function enrichFromWebsite(url, { fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT_MS, contact = DEFAULT_CONTACT, now = () => new Date() } = {}) {
  ...
  const html = await fetchText(url, { fetchImpl, timeoutMs, contact });
 ...
 */
export async function enrichFromOpenStreetMap({ nome, cidade }, { fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT_MS, contact = DEFAULT_CONTACT, url = process.env.DATTASELLER_DISCOVERY_NOMINATIM_URL || DEFAULT_NOMINATIM_URL, now = () => new Date() } = {}) {
  ...
  const payload = JSON.parse(await fetchText(endpoint.toString(), { fetchImpl, timeoutMs, accept: "application/json", contact }));
 ...
 */
export async function enrichLead({ lead, fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT_MS, contact = DEFAULT_CONTACT, now = () => new Date() } = {}) {
  ...
  if (siteUrl && isPublicHttpUrl(siteUrl)) {
    try {
      const site = await enrichFromWebsite(siteUrl, { fetchImpl, timeoutMs, contact, now });
  ...
  try {
    const osm = await enrichFromOpenStreetMap({ nome: lead.nome, cidade: lead.cidade }, { fetchImpl, timeoutMs, contact, now });
