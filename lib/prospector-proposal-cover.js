...
function renderCommercialScope({ productName, price, currency, terms, validUntil }) {
  const hasPrice = Number.isFinite(Number(price)) && Number(price) > 0 && typeof currency === "string" && currency.trim();
  const formattedPrice = hasPrice ? new Intl.NumberFormat("pt-BR", { style: "currency", currency }).format(Number(price)) : "Valor a confirmar com o operador";
  return `<section class="artifact"><div class="kicker">Escopo comercial</div><h2>Próximos passos</h2><div class="artifact-grid"><article class="artifact-card"><b>Serviço</b><p>${escape(productName || "Escopo personalizado")}</p></article><article class="artifact-card"><b>Investimento</b><p>${escape(formattedPrice)}</p></article><article class="artifact-card"><b>Condições</b><p>${escape(terms || "Condições a confirmar com o operador.")}</p></article><article class="artifact-card"><b>Validade</b><p>${escape(validUntil || "A confirmar")}</p></article></div></section>`;
}

export function renderProspectorProposalCover({ clientName, previewUrl, previewDocument, oldUrl, author, identity, whatsapp, productName, price, currency, terms, validUntil, diagnosisCriteria = [], socialAudits = [] }) {
