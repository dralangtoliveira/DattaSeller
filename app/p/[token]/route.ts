...
type ArtifactRefs = { preview_ids?: unknown; diagnosis_ids?: unknown; social_audit_ids?: unknown; public_proposal?: unknown; commercial_snapshot?: unknown };
...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: proposals, error } = await db.from("ds_proposals").select("id,lead_slug,valid_until,artifacts").contains("artifacts", { public_proposal: { token_hash: hash } }).limit(2);
  ...
  if (!previewIds.length || !diagnosisIds.length || !socialIds.length || !validateCommercialSnapshot(publishedArtifacts.commercial_snapshot)) return notFound();
  ...
  return new Response(renderProspectorProposalCover({ clientName: lead.data.nome || proposal.lead_slug, oldUrl: lead.data.site_antigo, diagnosisCriteria, socialAudits: social.data ?? [], publicMode: true, commercialSnapshot: publishedArtifacts.commercial_snapshot, validUntil: proposal.valid_until, previewHtml: sanitizePublicPreviewHtml(preview.data.content) }), { headers: publicHeaders() });
}
