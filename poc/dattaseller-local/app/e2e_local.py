...
p,e=email_flow('datta360','datta360',100); ds.email_transition(e['id'],'positive_reply'); o=ds.create_order(p['id']); ds.checkout(o['id'],'completed'); ds.payment(o['id'],'approved'); ct=ds.generate_contract(o['id']); ds.handoff(o['id'],'delivered')
assert ds.financial_summary()['revenue']==100 and ct['status']=='generated'

# DattaVPS mock
ds.qualify_lead('vps',['menção pública a servidor'],[],'dattavps','aderência observada','high','Qual carga?','proposta','Demo'); p2,e2=email_flow('vps','dattavps',190); ds.email_transition(e2['id'],'positive_reply'); o2=ds.create_order(p2['id']); ds.checkout(o2['id'],'completed'); ds.payment(o2['id'],'approved'); assert ds.handoff(o2['id'],'sent')['status']=='sent'

# Negativo: não cria receita, comissão ou handoff
pn,en=email_flow('negative','datta360',100); ds.email_transition(en['id'],'bounce'); on=ds.create_order(pn['id']); ds.checkout(on['id'],'abandoned'); assert ds.payment(on['id'],'declined')['status']=='declined'; assert ds.handoff(on['id'],'sent')['error']
print('E2E LOCAL OK: Datta360, DattaVPS mock e negativo')
