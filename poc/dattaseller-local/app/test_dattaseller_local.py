...
class DattaSellerLocalTests(unittest.TestCase):
    ...
    def test_order_checkout_payment_contract_handoff_commission_financial(self):
        ...
        self.assertEqual(ds.checkout(o['id'])['status'],'open'); self.assertEqual(ds.checkout(o['id'],'abandoned')['status'],'abandoned'); self.assertEqual(ds.checkout(o['id'],'completed')['status'],'completed'); self.assertEqual(ds.checkout(o['id'],'expired')['status'],'expired')
        ...
        self.assertEqual(ds.handoff(o['id'],'failed')['status'],'failed'); retried=ds.handoff(o['id'],'sent'); self.assertEqual((retried['status'],retried['retry_count']),('sent',1)); self.assertEqual(ds.handoff(o['id'],'delivered')['status'],'delivered')
