...
Verificação de Vercel (2026-09-18, somente leitura): `crm.datta360.com.br` aponta para `v0-project-6xwoozsra-datta-x.vercel.app` (Production, Ready, 2026-09-15 13:36 -04) e o site público para `datta360-r6c7uspwf-datta-x.vercel.app` (projeto `datta360`). Variáveis de Production do `v0-project` (somente nomes): `RESEND_API_KEY`, `DATTA360_WEBHOOK_SECRET`, `SUPABASE_SECRET_KEY` (Secret) e `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (Config, Preview+Production). Achado importante: a chave do Resend está **só em Production**, então um E2E em Preview não consegue provar o envio real sem decisão humana.
