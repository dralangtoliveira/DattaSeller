...
| HML | O conector Supabase identifica `dattaseller-hml` (ref `qfwvkarvueuezeqfljbl`) como `INACTIVE`; Production é outro projeto (ref `vkvkzoulbljampcbxaim`) e `ACTIVE_HEALTHY`. O conector Vercel devolve `403 Forbidden` para o escopo `datta-x`, mas a sessão autenticada no navegador mostra o projeto `v0-project`: deployment `dpl_DS2eoRCF4gh4TgG2s9gMP1svFywf` do commit `6e78a31`, branch canônica, ambiente `Preview`, estado `Ready`, URL `https://v0-project-jkg8m04ah-datta-x.vercel.app`. Na configuração Preview, a URL Supabase específica de HML está limitada à branch `codex/supervisor-dattaseller`; a variável `NEXT_PUBLIC_SUPABASE_URL` aplicável a `Production and Preview` aponta para o ref de Production `vkvkzoulbljampcbxaim`. Nenhum valor secreto foi revelado. |
| bloqueio | `PREVIEW_DB_NOT_ISOLATED`: o Preview canônico herda o banco de Production e não é alvo permitido do preflight HML. `HML_INACTIVE`: o projeto HML não está ativo. `DS_E2E_EMAIL_TO` não está registrado como caixa controlada; `DS_E2E_CONFIRM=yes` não foi fornecido para a execução. Provider real permanece `UNVERIFIED` (apenas a existência de `RESEND_API_KEY` em `Production and Preview` foi vista). Nenhum preflight real ou fluxo mutável foi executado. |
| próxima ação | Preparar um Preview com variáveis de projeto restritas à branch canônica e apontando exclusivamente ao ref HML, após autorização para a mudança ambiental; reativar o HML e registrar mailbox controlado. Confirmar novamente o binding do deployment criado e então executar somente `npm run e2e:preflight` com `DS_E2E_CONFIRM=yes` efêmero. |
